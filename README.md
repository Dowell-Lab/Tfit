...
| -bg    --bedgraph   | \<FILE.cat.bedGraph>      | Concatenated pos/neg bedGraph file (***Required only if -f and -r not specified***) 
| -N     --jobName    | \<MYJOB>                  | Job name for log output (no extensions).
| -o     --output     | \<FINALHIS.bed>           | User specified output name and directory for BED file (.bed extension)
| -l     --logOut     | \</project/logs/>         | Log file output directory. Will contain stdout (.log) and full model estimations (.tsv) with run specifications with --jobName as the rootname.
