...
def run(FILE):
	...
	with open(FILE) as FH:
		for line in FH:
			...
			if "#Data" in line:
				collect=True
			elif collect or "#" in line: #changed and to an or
				collect 	= False
				t 			= float(line[1:].split("\t")[0])#changed from ',' to '\t' also changed from int to float
				G[t] 		= list()
			elif collect:
				D.append(([float(x) for x in line.strip("\n").split("\t") ]))

			elif not collect or "#" in line: #changed this to an if statement from elif
			...
			if collect:
				...
				print(line)
				mu,si,l,w,pi 	= line.strip("\n").split(": ")[1].split(",")#list index out of range
...
if __name__ == "__main__":
	FILE 	= "/Users/jackdempsey/Desktop/Tfit_All/Tfit/python3_src/iterates.tsv"
	G, D 	= run(FILE)#g is empty
	print(G,"break\n")
	plot(G, D)
