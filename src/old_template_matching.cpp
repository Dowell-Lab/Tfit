...
//These parameters have effectively never changed:
	...
	#pragma omp parallel num_threads(threads)
	{
		...
		for (int i = start; i < stop; i++){
			...
			if (k < data->XN  and j < data->XN and k!=j and N_neg > 0 and N_pos > 0 ){
				...
				BIC_values[i] 	= BIC3_old_long(data->X,  j,  k,  i, N_pos,  N_neg,avgLL, 
...
void run_global_template_matching_old_long(vector<segment*> segments, 
	...
	for (int i = 0; i < segments.size(); i++){
		...
		BIC_template_old_long(segments[i], avgLL, BIC_values, densities, densities_r, variances, lambdas,
